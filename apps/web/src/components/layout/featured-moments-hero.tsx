...
export function FeaturedMomentsHero({ slides, className }: FeaturedMomentsHeroProps) {
  ...
  return (
    <div
      className={cn("relative overflow-hidden rounded-xl border border-border/65 bg-muted/15", className)}
      onMouseEnter={() => setIsPaused(true)}
      onMouseLeave={() => setIsPaused(false)}
      onFocusCapture={() => setIsPaused(true)}
      onBlurCapture={() => setIsPaused(false)}
      aria-label="Featured moments from the road"
    >
      <div className="relative h-50 sm:h-68 lg:h-84">
        {hasMultipleSlides ? (
          <div className="pointer-events-none absolute inset-x-0 top-0 z-30 h-0.5 bg-black/22">
            <div
        ...
        {slides.map((slide, index) => {
          ...
          return (
            ...
            >
              ...
              <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(156deg,rgba(36,48,40,0.2),rgba(36,48,40,0.22)_48%,rgba(36,48,40,0.58)_100%)]" />
            </div>
          );
        })}

        <div className="absolute inset-x-0 bottom-0 z-20 p-4 sm:p-5">
          <p className="text-[11px] font-medium tracking-[0.18em] text-primary-foreground/90 uppercase">Featured moment from the road</p>
          <h2 className="mt-2 text-lg leading-tight font-semibold text-primary-foreground sm:text-xl">{activeSlide?.title}</h2>
